...
<b>If interested in the results, read
the [Full Report](https://www.researchgate.net/publication/348993267_An_Analysis_of_the_Performance_of_Websockets_in_Various_Programming_Languages_and_Libraries)
...
Total requests: num_clients × num_requests_per_client

Example: 10 clients × 5 requests each = 50 total requests
Time span: time_elapse is the duration from first request start to last request finish

Example: 2500ms total time span
Average calculation: time_elapse ÷ total_requests

Example: 2500ms ÷ 50 requests = 50ms average